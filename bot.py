...
@bot.tree.command(name='clm_export', description='Vytvoří import čekajících EP odměn pro WoW CLM Bridge')
async def clm_export(interaction: discord.Interaction):
    ...
    c=db()
    rows=c.execute('''SELECT a.id, a.character_name, o.reward AS ep, a.order_id
                      FROM clm_awards a
                      JOIN orders o ON o.id=a.order_id
                      WHERE a.guild_id=? AND a.status='PENDING'
                      ORDER BY a.id ASC LIMIT 50''', (interaction.guild_id,)).fetchall()
    c.close()
    if not rows:
        return await interaction.response.send_message('ℹ️ Nejsou žádné čekající CLM EP odměny.', ephemeral=True)
    parts=[]
    for r in rows:
        safe_name=str(r['character_name']).replace('|','').replace(';','').replace(',','').strip()
        parts.append(f"{safe_name},{int(r['ep'])},ORDER-{int(r['order_id'])}")
    payload='GOB1|'+';'.join(parts)
    if len(payload) > 1800:
        return await interaction.response.send_message('❌ Export je příliš dlouhý pro jednu Discord zprávu.', ephemeral=True)
    await interaction.response.send_message(f'📤 **CLM export – {len(rows)} čekajících odměn**\nVe WoW použij `/gobimport` a za něj vlož tento kód:\n```{payload}```\nBridge V0.6 bude každé ORDER-ID hlídat proti dvojímu připsání.', ephemeral=True)

@bot.tree.command(name='clm_confirm_all', description='Vedení: označí všechny aktuálně čekající CLM odměny jako importované')
async def clm_confirm_all(interaction: discord.Interaction):
    ...
    c=db()
    rows=c.execute("SELECT id, order_id FROM clm_awards WHERE guild_id=? AND status='PENDING' ORDER BY id ASC",
    ...
    now=datetime.now(timezone.utc).isoformat()
    c.execute("UPDATE clm_awards SET status='IMPORTED', processed_at=? WHERE guild_id=? AND status='PENDING'",
              (now, interaction.guild_id))
    c.commit()
    count=len(rows)
    c.close()
    await interaction.response.send_message(
        f'✅ Potvrzeno {count} CLM EP odměn jako IMPORTED. `/clm_export` je už nebude nabízet.',
        ephemeral=True
    )
