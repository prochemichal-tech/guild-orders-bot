...
def queue_clm_award(guild_id, user_id, character_name, ep, order_id):
    c=db()
    c.execute('''INSERT OR IGNORE INTO clm_awards
                 (guild_id,user_id,character_name,ep,order_id,status,created_at)
                 VALUES(?,?,?,?,?,'PENDING',?)''',
              (guild_id,user_id,character_name,ep,order_id,datetime.now(timezone.utc).isoformat()))
    c.commit(); c.close()
...
async def clm_export(interaction: discord.Interaction):
    ...
    c=db()
    rows=c.execute("SELECT id, character_name, ep, order_id FROM clm_awards WHERE guild_id=? AND status='PENDING' ORDER BY id ASC LIMIT 50", (interaction.guild_id,)).fetchall()
    c.close()
    if not rows:
