    ...
    class MultiOrderView(discord.ui.View):
        ...
        async def fulfill(self, button_interaction: discord.Interaction, button: discord.ui.Button):
            ...
            await button_interaction.response.send_message(
                f'✅ Vzal sis společný order **{item} ×{quantity}** za **{reward} EP**. '
                f'Vytvořil se ti vlastní order **#{oid:03d}**, který funguje stejně jako běžný order.',
                ephemeral=True)
            try:
                msg = await button_interaction.channel.send(
                    content=f'👤 Splnění společného orderu: {button_interaction.user.mention}',
                    embed=await embed_for(button_interaction.guild, child),
                    view=OrderView(oid))
                set_message(oid, msg.id)
            except discord.HTTPException as e:
                print(f'Společný order #{oid}: chyba vytvoření karty: {type(e).__name__}: {e}')
