...
WEEKLY_POINT_LIMIT = int(os.getenv('WEEKLY_POINT_LIMIT', '50'))
if not TOKEN:
...
async def embed_for(g, o):
    ...
    e.set_footer(text='Guild Orders • EP se zatím připisují ručně')
    return e
        ...
        async def callback(interaction):
            ...
            await interaction.response.edit_message(
                content=(
                    ...
                    f'(nový stav: **{new_inventory_quantity} ks**). EP lze nyní ručně doplnit.'
                ),
                view=None
            )
            ...
            try:
                ...
                dm.add_field(name='Informace', value='EP ti budou připsány.', inline=False)
...
async def order_multi(interaction: discord.Interaction, item: str, quantity: int, reward: int):
    ...
    class MultiOrderView(discord.ui.View):
        ...
        @discord.ui.button(label='🙋 SPLNIT ORDER', style=discord.ButtonStyle.success)
        async def fulfill(self, button_interaction: discord.Interaction, button: discord.ui.Button):
            ...
            oid = create_order(item, quantity, reward, interaction.user.id, button_interaction.channel_id, button_interaction.guild_id)
            c=db()
            ...
            await button_interaction.response.send_message(
                f'✅ **{button_interaction.user.display_name}** se přihlásil ke splnění **{item} ×{quantity}** za **{reward} EP**.\n'
                f'Použij `/orders` pro dohledání svého orderu a dokončení. Tento společný order zůstává otevřený pro další hráče.',
                ephemeral=True)
            # Notify staff in channel so reviewer immediately knows who fulfilled it.
            try:
                await button_interaction.channel.send(
                    f'🔔 **Nové splnění společného orderu:** {button_interaction.user.mention} — '
                    f'**{item} ×{quantity}** za **{reward} EP**. Interní order **#{oid:03d}** čeká na dokončení hráčem.'
                )
    ...
    e=discord.Embed(
        ...
        description='Tento order může splnit více hráčů a zůstává aktivní, dokud ho vedení ručně nesmaže.',
        color=discord.Color.green())
